...
function ChartCard({ spec, all }: { spec: ChartSpec; all: Series[] }) {
  ...
  useEffect(() => {
    ...
    const timeMarker = currentDate
      ? [Plot.ruleX([currentDate], { stroke: '#DDAE5C', strokeWidth: 1.5, strokeOpacity: 0.8 })]
      : []
    ...
    const base = {
      ...
      marginTop: 12,
      marginBottom: 28,
      style: { background: 'transparent', color: '#9aa4ab', fontSize: '10px' },
      grid: true,
    }
    ...
    if (spec.type === 'line') {
      ...
      chart = Plot.plot({
        ...
        marks: [
          ...
          ...timeMarker,
          Plot.ruleY([0], { stroke: '#39424a', strokeOpacity: 0.4 }),
        ],
      })
      ...
      chart = Plot.plot({
        ...
        marks: [
          ...
          Plot.linearRegressionY(rows, { x: 'x', y: 'y', stroke: '#7FB3A3', strokeOpacity: 0.6 }),
        ],
      })
    }
